const offset = 11 //custom_items + data driven items
const aux = 65536
...
categories.set("Skyblock Path", [65536 * 2, `\
cobblestone
title: Cobble Gen
icon: 262144
reward: give @s cobblestone 32; 32 Cobblestone
...
reward: give @s oak_log 2; 2 Oak Logs
...
reward: give @s wheat 16; 16 wheat
...
- build a bridge to the snowy taiga, jungle, mangrove swamp, or desert island
- stand on the new island
- complete the achievement for the island you visited in the explorer path after completing this achievement
...
reward: give @s iron_ingot; 1 Iron Ingot
* Build a mob trap
- collect a bunch of building blocks
- build a 20 blocks tall 2x2 drop shoot
- build a water stream to each cardinal direction of your drop shoot
...
reward: give @s bed 1; Another Bed for your friend
...
reward: give @s cobblestone_wall 64; A stack of Walls
...
reward: give @s gold_ingot 8; 8 Gold Ingots
...
icon: 196608
* Covert a stack of gravel into a stack of dirt
...
title: Deadly Fort
icon: 7340032
reward: give @s cooked_beef 3; 3 Steaks
...
reward: give @s diamond 2; 2 More diamonds

brewing
require: blaze_rod
title: Alchemist
icon: textures/items/potion_bottle_weakness
reward: give @s gunpowder 32; 32 Gunpowder
...
reward: give @s emerald 20; 20 Emeralds
...
reward: Enchant the next tool or armor you hold with mending
...
reward: give @s oak_slab 256; 4 stacks of slabs
...
title: Shell Lurkers
icon: 13434880
reward: give @s shulker_shell 4; 4 Shulker Shells
...
reward: give @s firework_rocket 16; 16 Flight Rockets
...
categories.set("Explorer Path", ['textures/items/compass_item', `\
swamp
require: bridge
title: Stinky Woods
reward: summon frog; A Frog
...
reward: give @s dark_oak_sapling 3; 3 Dark Oak Saplings
...
reward: give @s mushroom_stew 4; 4 Mushroom Stew
...
reward: give @s redstone_torch; A redstone torch
...
reward: effect @s bad_omen 600; Bad Omen for 10 minutes 
...
title: Trials & Errors
icon: -49807360
reward: give @s waxed_copper 10; 10 Copper Blocks
...
* Raid the nether bastian while wearing any golden armor
...
require: (biome_detector & soulsand_valley & locked)
title: Nio Diversity
icon: 14155776
...
`])
categories.set("Farmer Path", ['textures/items/diamond_hoe',  `\
wheat_farm
require: farm
title: Bread Fields
icon: textures/items/wheat
consume: wheat 128; 2 Stacks of Wheat
reward: give @s bone_block 16; 16 Bone Blocks
...
consume: [carrot 64, potato 64]; 64 Carrots and 64 Potatoes
reward: Fortune II Golden Hoe
...
icon: 5308416
consume: cactus 64; A stack of Cactus
reward: xp 128; The xp you get from smelting 10 stacks of cactus
* Build an cactus farm
...
icon: 786432
consume: sand 64; 64 Blocks of Sand
reward: give @s oak_leaves 256; 4 stacks of Oak Leaves
* Build a husk farm in the desert biome to farm sand
- build a large platform in the desert biome for husks to spawn at night
- use water to flush the mobs into a drop shoot
...
icon: 5636096
consume: pumpkin 64; 64 Pumpkins
reward: give @s shears; Shears
...
(Selling Pumpkins to villagers is a great source of emeralds; This image shows the best layout for farming melons and pumpkins; You may automate the farm later with pistons and observers)
...
icon: 5242880
consume: snow 64; 64 Snow Blocks
reward: Unbreaking III Iron shovel
...
consume: [red_mushroom 32, brown_mushroom 32]; 32 Brown and 32 Red Mushrooms
reward: give @s iron_axe 3; 3 Iron Axes
...
(Slimes can only spawn below Y level 0)
...
icon: -20119552
reward: give @s silence_armor_trim_smithing_template; Silence Armor Trim
...
reward: scriptevent skybedrock:unlock_vision yes; Unlock Over 16 chunks render distance
* Visit all 15 the structures on the map and complete their dedicated acheievements
- Source of Evil
- Tomb Raider
- Snow House
- Welcome to Gize
...
reward: A treasure map and a Banner
...
export const queries = {
    cobblestone: (player) => check_items(player, 'cobblestone', 64),
    sapling: (player) => ['oak', 'spruce', 'birch', 'jungle', 'acacia', 'cherry'].some(sapling => 
        check_items(player, sapling +'_sapling', 3)
    ),
    farm: (player) => view_stats(player, 'blocks_placed', 'minecraft:wheat') >= 3,
    animals: (player) => ['cow', 'chicken', 'pig', 'sheep'].some(animal =>
    ...
    // water,
    mob_trap: (player) => check_items(player, 'bone', 2) && check_items(player, 'gunpowder', 2) && check_items(player, 'string', 2),
    ...
    fortress: (player) => check_location(player, 'minecraft:nether', [{x:64, y: 50, z: 113}, {x:111, y: 55, z: 143}]),
    blaze_rod: (player) => check_items(player, 'blaze_rod', 2),
    brewing: (player) => check_items(player, 'splash_potion', null, 34),
    diamond: (player) => check_items(player, 'diamond'),
    villagers: (player) => player.runCommand('testfor @e[type=villager, r=5]').successCount > 1,
    iron: (player) => player.runCommand('testfor @e[type=iron_golem]').successCount && check_items(player, 'iron_ingot', 32),
    saddle: (player) => {
    ...
    lodestone: (player) => check_items(player, 'lodestone_compass'),
    diamond_pickaxe: (player) => player.runCommand('testfor @e[type=villager, r=5]').successCount && check_items(player, 'diamond_pickaxe', null, 0),
    silk_touch: (player) => ['wooden', 'golden', 'stone', 'iron', 'diamond', 'netherite'].some(type =>
        check_item_enchants(player, `minecraft:${type}_pickaxe`, 'silk_touch')
    ),
    ender_eye: (player) => view_stats(player, 'items_used', 'minecraft:ender_eye'),
    stronghold: (player) => (
        player.dimension.id == "minecraft:overworld" &&
        [1, 2, 3].some(i => {
    ...
    the_end: (player) => check_location(player, 'minecraft:the_end'),
    dragon: (player) => check_items(player, 'dragon_head') && view_stats(player, 'mobs_killed', 'minecraft:ender_dragon'),
    shulkers: (player) => check_items(player, 'undyed_shulker_box') && player.runCommand('testfor @e[type=shulker]').successCount > 1,
    end_pillars: () => pillar_locations.every(loc => check_block(world.getDimension("minecraft:the_end"), {...loc, y: 0}, "minecraft:obsidian", false)),
    end_phantom: (player) => view_stats(player, 'mobs_killed', 'skybedrock:end_phantom'),
    elytra: (player) => player.isGliding,
    
    swamp: (player) => in_raduis(player, '96 63 0'),
    jungle: (player) => in_raduis(player, '-96 63 0'),
    taiga: (player) => in_raduis(player, '0 63 -96'),
    desert: (player) => in_raduis(player, '0 63 96'),
    savanna: (player) => in_raduis(player, '96 63 96'),
    dark_forest: (player) => in_raduis(player, '96 63 -96'),
    birch: (player) => in_raduis(player, '-96 63 -96'),
    mushroom_island: (player) => in_raduis(player, '-96 63 96'),
    ocean: (player) => in_raduis(player, '192 63 0'),
    badlands: (player) => in_raduis(player, '0 63 192'),
    resin_clump: (player) => in_raduis(player, '0 63 -192'),
    cherry: (player) => in_raduis(player, '-192 63 0'),
    dripstone: (player) => in_raduis(player, '45 31 -145'),
    geode: (player) => in_raduis(player, '-45 32 145'),
    lush_cave: (player) => in_raduis(player, '-145 31 -49'),
    deep_dark: (player) => in_raduis(player, '145 -33 49'),
    crimson: (player) => in_raduis(player, '96 63 0', 'minecraft:nether'),
    warped: (player) => in_raduis(player, '-96 63 0', 'minecraft:nether'),
    soulsand_valley: (player) => in_raduis(player, '0 63 -96', 'minecraft:nether'),
    basalt_deltas: (player) => in_raduis(player, '0 63 96', 'minecraft:nether'),
    biome_detector: (player) => player.getDynamicProperty('biome_detector'),
    structure_locator: (player) => locating_players.has(player.id),
    witch_hut: (player) => (
        in_raduis(player, '819 65 356', undefined, 10) && 
        !player.runCommand('testfor @e[type=witch, r=32]').successCount
    ),
    jungle_temple: (player) => {
    ...
    ruined_portal: (player) => check_items(player, 'netherite_upgrade_smithing_template'),
    end_city: (player) => in_raduis(player, '-298 71 -954', 'minecraft:the_end', 8),
    located_all_structures: (player) => [
        ...
        'fortress_loot', 'bastian', 'ruined_portal',
...
export const rewards = {
    ...
    direwolf: (player) => player.dimension.spawnItem(stored_items.rewards[0], player.location),
    pirate: (player) => {
