...
const breath_particle = new MolangVariableMap()
breath_particle.setFloat("variable.cloud_lifetime", 1)
breath_particle.setFloat("variable.cloud_radius", 1)
breath_particle.setFloat("variable.particle_multiplier", 10)
...
function breathe(block, direction, iterations) {
  iterations--
  block.dimension.spawnParticle("minecraft:dragon_breath_lingering", block.center(), breath_particle)
...
world.afterEvents.pistonActivate.subscribe(({block, piston}) => {
  ...
  if (dragon_breath) {
    ...
    let consumed = false
    for (let i = 0; i < hopper_container.size; i++) {
      let item = hopper_container.getItem(i)
      if (!item) continue
      if (item.typeId != "minecraft:dragon_breath") continue
      item.amount > 1 ? item.amount-- : item = undefined
      hopper_container.setItem(i, item)
      consumed = true; break
    }
    if (!consumed) return
  } else return
